...
setProperty() {
	...
	if [ -f "/usr/local/tomcat/webapps/applicationNavigator/WEB-INF/web.xml" ]; then
		if [ "$prop" = "banner9.baseurl" ]; then
			for settings_file in /usr/local/tomcat/webapps/applicationNavigator/WEB-INF/web.xml; do
