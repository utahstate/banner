...
setProperty() {
  ...
  #Enable Theme for BannerAdminBPAPI
#  if [ "$prop" = "theme.url" ]; then
#    sed -i "2i <param name=\"APP_CSS_URL\" value=\"$val\" />\\n<param name=\"APP_CSS_APPEND\" value=\"true\" />" /usr/local/tomcat/webapps/BannerAdminBPAPI/WEB-INF/classes/config.xml
#  fi

  #Set CAS server for BannerAdminBPAPI
#  if [ "$prop" = "cas.url" ]; then
#    sed -i "s|^cas\.server\.location.*|cas\.server\.location = $val|g" /usr/local/tomcat/webapps/BannerAdminBPAPI/WEB-INF/classes/config.properties
#  fi
